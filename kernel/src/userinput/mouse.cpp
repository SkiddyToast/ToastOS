...
void InitPS2Mouse()
{
    outb(0x64, 0xA8); //Enable PS2 mouse
    MouseWait();
    outb(0x64, 0x20); //Tells the keyboard we want to communicate with the mouse
    MouseWaitInput();
    ...
    MouseWrite(0xF6);
    MouseRead();
    MouseWrite(0xF4);  //Telling the mouse we want to use default settings
    MouseRead();
}